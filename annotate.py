...
def count_performance_rows():
    """Count how many times the loan appears on the "performance dataset, and check if if was foreclosed on"""
    counts = {}
    with open(os.path.join(settings.PROCESSED_DIR, "Performance.txt"), 'r') as f:
        for i, line in enumerate(f):
            ...
            if loan_id not in counts:
                # If the loan is not in the "counts" dictionary, add it
                counts[loan_id] = {
            ...
            if len(date.strip()) > 0:
                # If the date is not None, the loan was closed on
                counts[loan_id]["foreclosure_status"] = True
    return counts


def get_performance_summary_value(loan_id, key, performance_summary):
    """Return the key corresponding to "loan_id" if it exists on the "performance_summary" dictionary, or a default
    value if it doesn't """
...
def annotate(acquisition, performance_summary):
    # Create foreclosure status and performance columns
    ...
    # Keep only the loans that where kept for a minimum amount of quarters
